  	...
  	function($scope, Bases, Base, Apy, PublicApy, $upload, $cookies, $window,
  		...
  		$scope.submit_new_base = function(e) {
  			...
  			$http({
  				...
  				$document.find('form#new_base div').append(angular.element(
          ...
          */
  		...
  		$scope.save = function(apy) {
  			Apy1.update({
  				...
  				$scope.alerts.push({
  					type: 'success',
  					msg: "Exec '" + apy.name + "' saved"
  				});
  			}, function(data) {
  				$scope.alerts.push({
  					type: 'danger',
  					msg: "Exec '" + apy.name + "' not saved"
  				});
  				console.error("error");
  				console.error(data);
  				angular.forEach(data.data.detail.errors, function(value, key) {
  					$scope.alerts.push({
  						type: 'danger',
  						msg: value.filename + ":" + value.lineno + ":" + value.col +
  							": " + value.msg
  					});
  				});
  		...
  		$scope.printcurl = function(apy) {
  			...
  			add_client_message("user:   curl -u " + window.username +
  				" -H'Cookie: " + document.cookie + "' \"" + parser.protocol + "//" +
  		...
  		$scope.rename = function($event) {
  			new_exec_name = $($event.currentTarget.parentNode.parentNode).find(
  				'input').first().val();
  ...
  window.app.controller('TransactionCtrl', ['$scope', '$dragon', function(
  	...
  	$dragon.onReady(function() {
  		...
  		$dragon.subscribe('logentry-router', $scope.channel_logs, {}).then(
  			function(
  				response) {
  	...
  	$dragon.onChannelMessage(function(channels, message) {
  		...
  		if (indexOf.call(channels, $scope.channel) > -1) {
  			...
  			$scope.$apply(function($scope) {
  				$scope.dataMapper.mapData($scope.transactions, message);
  			});
  		}

  		if (indexOf.call(channels, $scope.channel_logs) > -1) {
  			$scope.$apply(function($scope) {
  ...
  window.app.controller('TransportEndpointCtrl', ['$scope', '$filter', '$q',
  	...
  	function($scope, $filter, $q, $http, $timeout, TransportEndpoints, Base) {
